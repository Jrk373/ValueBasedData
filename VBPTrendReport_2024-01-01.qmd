...
These are excel files and require importing of a specific sheet. The name of the sheet has varied in name and location within the file, sometimes appearing a literal sheet, and sometimes appearing only in the data model view. In addition, the format of the file changed from 2022-2023, requiring a reworking of the wrangle code. 
...
# Replace './data/Master' with your folder path
folder_path <- "./data/VBPReports/Master/2023"
...
Leaving off here. Somehow, the measurement year, report period, etc. have to be recombined from the Controls sheet with the Roster sheet.

```{r}
# Now do all of that for files with "Roster" as the sheet of interest

# Filter list items that end in "_Roster"
filtered_list <- result_separate_df[sapply(names(result_separate_df), function(x) grepl("_Roster$", x))]

# Bind tibbles/data frames into one single data frame
combined_df_Roster <- bind_rows(filtered_list)
...
# Store the values in a list called DetailSheetNames
RosterSheetNames <- as.list(first_row_values)
...
filtered_combined_df <- 
  filtered_combined_df |> 
  filter(`BCBSAZ Health Choice` != "Measure")

# Convert DetailSheetNames to character vector
RosterSheetNames <- as.character(RosterSheetNames)

# Rename columns using values from DetailSheetNames
names(filtered_combined_df) <- RosterSheetNames
