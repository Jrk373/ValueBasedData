...
# merged_df <- merged_df |>
#   mutate(`Gap Status` = ifelse(`Measure ID` == "AMM" & `Gap Status` == 0, "OPEN", "CLOSED"))
...
counts_table <- 
  ...
  mutate(
    Percentage_Closed = scales::percent(Closed_Count / Total_Count, accuracy = 0.01)
  )
...
# Convert Percentage_Closed to numeric without "%" sign
counts_table$Percentage_Closed <- as.numeric(gsub("%", "", counts_table$Percentage_Closed))

# Create separate line charts for Percentage_Closed over ClaimsAdjudicatedThrough per ProviderShortname for each MeasureID
plots_list <- list()

for (measure_id in unique(counts_table$MeasureID)) {
  plot_data <- counts_table |>
    filter(MeasureID == measure_id)
  
  # Check if Percentage_Closed column is a character type, convert it to numeric
  if (class(plot_data$Percentage_Closed) == "character") {
    plot_data$Percentage_Closed <- as.numeric(gsub("%", "", plot_data$Percentage_Closed))
  }
  
  plot <- ggplot(plot_data, aes(x = ClaimsAdjudicatedThrough, y = Percentage_Closed, group = ProviderShortname)) +
    geom_line(aes(color = ProviderShortname), size = 1) +
    labs(
      title = paste("Percentage Closed over Time per Provider for Measure ID", measure_id),
      x = "Claims Adjudicated Through",
      y = "Percentage Closed"
    ) +
    theme_minimal() +
    scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 10)) +
    geom_hline(yintercept = c(57, 45, 35, 93), linetype = "dashed", color = "red")
  
  plots_list[[paste0("Plot_", measure_id)]] <- plot
}

# Show the separate line plots for each MeasureID
plots_list
...
# Creating a table that counts occurrences of "OPEN" and "CLOSED" per group
counts_table <- Data_VBPQR_AllRecords |>
  group_by(ClaimsAdjudicatedThrough, MeasureID) |>
  summarise(
    Open_Count = sum(GapStatus == "OPEN", na.rm = TRUE),
    Closed_Count = sum(GapStatus == "CLOSED", na.rm = TRUE),
    Total_Count = Open_Count + Closed_Count
